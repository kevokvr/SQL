...
select distinct * from Actors order by actorId;
-- query 02) return all actresses sorted by actorName
select * from Actors where sex = 'F';
Select (select count(sex) from Actors) as "Total", (select count(sex) from Actors where sex = "m") as "Male", (select count(sex) from Actors where sex = "f") as "female";
select count(*) from Actors where sex = 'F';
-- query 04) return the names of the actors/actresses that were in 'The Americans' sorted by actorName
select actorName from Actors A INNER JOIN acts B on A.actorId = B.actorId INNER JOIN Series C on C.seriesId = B.seriesId WHERE title = 'The Americans' order by actorName;
